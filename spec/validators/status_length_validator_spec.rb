...
describe StatusLengthValidator do
  describe '#validate' do
    ...
    it 'adds an error when content warning is over 1000 characters' do
      status = instance_double(Status, spoiler_text: 'a' * 1020, text: '', errors: activemodel_errors, local?: true, reblog?: false)
      subject.validate(status)
      expect(status.errors).to have_received(:add)
    end

    it 'adds an error when text is over 1000 characters' do
      status = instance_double(Status, spoiler_text: '', text: 'a' * 1020, errors: activemodel_errors, local?: true, reblog?: false)
      subject.validate(status)
      expect(status.errors).to have_received(:add)
    end

    it 'adds an error when text and content warning are over 1000 characters total' do
      status = instance_double(Status, spoiler_text: 'a' * 500, text: 'b' * 501, errors: activemodel_errors, local?: true, reblog?: false)
      subject.validate(status)
      expect(status.errors).to have_received(:add)
    end
    ...
    it 'does not count overly long URLs as 23 characters flat' do
      text = "http://example.com/valid?#{'#foo?' * 1001}"
      status = instance_double(Status, spoiler_text: '', text: text, errors: activemodel_errors, local?: true, reblog?: false)
      subject.validate(status)
      expect(status.errors).to have_received(:add)
    end
